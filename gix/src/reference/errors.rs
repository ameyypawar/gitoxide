...
pub mod edit {
    ...
    //                impls. (`init::Error::EditHeadForDefaultBranch` at `gix/src/init.rs:39` has no
...
///
pub mod peel {
    // TODO(review): kept concrete. Matched at `gix/src/remote/connection/fetch/update_refs/mod.rs:212`
    //                to detect unborn refs: `Error::ToId(gix_ref::peel::to_id::Error::
    //                FollowToObject(gix_ref::peel::to_object::Error::Follow(_)))`. Its sole parent,
    //                `head::peel::Error::PeelReference` (`gix/src/head/peel.rs:18`), has no erased
    //                member.
...
pub mod find {
    ...
    pub mod existing {
        ...
        // TODO(review): kept concrete. Callers match its `NotFound` variant directly:
        //                `gix/src/repository/reference.rs:193` and
        //                `gix/src/revision/spec/parse/delegate/revision.rs:210`. Separately,
        //                `repository::index_or_load_from_head_or_empty::Error::ReadHead`
        //                (`gix/src/repository/mod.rs:314`) already has an erased slot via
        //                `PeelToTree` (`:318`), so this type is doubly blocked from erasure there.
